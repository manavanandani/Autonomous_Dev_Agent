...
- **LLM Integration**: OpenAI and Anthropic models for various agent tasks
...
Run the agent with a requirements file:

```bash
python -m src.main --requirements path/to/requirements.txt --output output_directory
```

Or provide requirements interactively:

```bash
python -m src.main --output output_directory
```
...
```bash
python -m src.main --requirements path/to/requirements.txt --output output_directory --vcs github
```

To use with GitLab:

```bash
python -m src.main --requirements path/to/requirements.txt --output output_directory --vcs gitlab
```
